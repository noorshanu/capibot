...
const HOLDERS: React.FC = () => {
  const [, setTonConnect] = useState<TonConnect | null>(null);
  ...
  const tokenProps = useSelector(state => state.tokens.tokens);
  const tokens = tokenProps?.filter((t) => t.type === 'HOLDERS');
  ...
  useEffect(() => {
    const initTonConnect = async () => {
      try {
        const tc = new TonConnect({ manifestUrl: 'https://www.tapnot.app/tonconnect-manifest.json' });
  ...
  const handleBuyToken = async (token: tokenProps) => {
